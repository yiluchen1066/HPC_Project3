...
{
    double result = 0;
    #pragma omp parallel for reduction (+: result) //schedule(static) shared(x, y) default(none) firstprivate(N)
    for (int i = 0; i < N; i++)
        result += x[i] * y[i];
...
{
    double result = 0;
    #pragma omp parallel for reduction (+: result) //schedule(static) shared(x) default(none) firstprivate(N) 
    for (int i = 0; i < N; i++)
    {
        double xi = x[i];
        result += xi*xi; 
